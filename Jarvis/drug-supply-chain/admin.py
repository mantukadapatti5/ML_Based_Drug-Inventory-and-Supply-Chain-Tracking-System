...
def admin_dashboard_stats(
    ...
    try:
        ...
        pending = db.execute(text("SELECT COUNT(*) FROM users WHERE verified = 0")).scalar() or 0
        anomalies = db.execute(text("SELECT COUNT(*) FROM anomaly_logs WHERE resolved = 0")).scalar() or 0
    except Exception:
        users_count, orders_count, drugs_count, pending, anomalies = 0, 0, 0, 0, 0
...
def compliance_report_json(
    ...
    try:
        ...
        cold_alerts = db.execute(
            text("SELECT COUNT(*) FROM anomaly_logs WHERE anomaly_type LIKE '%TEMP%' AND resolved = 0")
        ).scalar() or 0
    except Exception:
