...
def _days_until(expiry_date_val) -> int:
    """Calculate days until expiry — works for both string and date objects."""
    try:
        if isinstance(expiry_date_val, (date, datetime)):
            exp = expiry_date_val if isinstance(expiry_date_val, date) else expiry_date_val.date()
        else:
            exp = datetime.strptime(str(expiry_date_val)[:10], "%Y-%m-%d").date()
...
async def update_inventory_item(item_id: int, item: ItemCreate, db: Session = Depends(get_db)):
    try:
        db.execute(
            text("UPDATE drugs SET name=:name, batch_no=:batch_no, manufacturer=:manufacturer, quantity=:quantity, price=:price, expiry_date=:expiry_date WHERE id=:id"),
...
# ═══════════════════════════════════════════════════════════════════════════
# POST /api/inventory/request-stock  ← Distributor → Vendor
# ═══════════════════════════════════════════════════════════════════════════
...
async def request_stock(payload: StockRequestPayload, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "drug_id": payload.drug_id, "quantity": qty, "status": "PENDING",
...
async def update_request_status(req_id: int, status: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"success": True, "request_id": req_id, "new_status": status_upper, "updated_at": datetime.utcnow().isoformat()}
...
# ═══════════════════════════════════════════════════════════════════════════
# GET /api/inventory/fefo-sorted  — PostgreSQL + SQLite compatible
# ═══════════════════════════════════════════════════════════════════════════
@router.get("/inventory/fefo-sorted")
async def get_fefo_sorted(limit: int = 20, db: Session = Depends(get_db)):
    # PostgreSQL uses NOW() and DATE arithmetic; SQLite uses julianday
    try:
        if settings.is_postgres:
            rows = db.execute(text("""
                SELECT id, batch_id, drug_id, drug_name, expiry_date, quantity_units, storage_zone,
                       CAST(expiry_date - CURRENT_DATE AS INTEGER) AS days_until_expiry
        ...
        if rows:
            ...
            for i, r in enumerate(rows, 1):
                d = dict(r)
                d["fefo_rank"] = i
                # Recalculate days using Python for accuracy
    ...
    # Fallback: read from drugs table (PostgreSQL compatible)
    try:
        rows = db.execute(text("""
            SELECT id, batch_no AS batch_id, name AS drug_name,
                   expiry_date, quantity AS quantity_units
            ...
            return {
                "batches": [
                    {**dict(r), "fefo_rank": i, "storage_zone": "WH-A",
                     "days_until_expiry": _days_until(dict(r).get("expiry_date"))}
                    for i, r in enumerate(rows, 1)
                ]
            }
    except Exception:
...
# ═══════════════════════════════════════════════════════════════════════════
# GET /api/inventory/rop-dashboard
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# POST /api/inventory/calculate-rop
# ═══════════════════════════════════════════════════════════════════════════
@router.post("/inventory/calculate-rop")
async def calculate_rop(data: dict, db: Session = Depends(get_db)):
    return {
        "rop": 150, "safety_stock": 50, "economic_order_qty": 300,
        "avg_daily_demand": 10, "lead_time_days": 5,
        "drug_id": data.get("drug_id"), "region": data.get("region", "Ahmedabad"),
    }
